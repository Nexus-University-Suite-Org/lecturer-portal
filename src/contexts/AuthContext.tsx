...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = async (
    ...
    try {
      ...
      const userRole =
        (response.user.role as Profile["role"]) ??
        (response.profile.role as Profile["role"]) ??
        "lecturer";
      ...
      const profile: Profile = {
        ...
        department: response.profile.assignedProgramme ?? response.profile.department ?? null,
        college: response.profile.college ?? null,
        programme: response.profile.programme ?? response.profile.programChoice1 ?? null,
        phone: response.profile.phoneNumber ?? null,
        phone_number: response.profile.phoneNumber ?? null,
        bio: null,
        role: userRole === "student" ? "lecturer" : userRole,
      };
